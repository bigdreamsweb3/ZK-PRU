...
| Phishing site requests a look-alike signature under a different domain | Both fixed challenges are EIP-712 typed data bound to `verifyingContract` (see [`03-identity-model.md`](./03-identity-model.md)); wallets that surface EIP-712 domain fields give the user a concrete mismatch signal |
